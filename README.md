...
`gemini-ocr-tool` is a Python CLI tool built with modern tooling and best practices.
...
```bash
# Show help
gemini-ocr-tool --help

# Run the tool
gemini-ocr-tool

# Run with verbose output
gemini-ocr-tool -v      # INFO level
gemini-ocr-tool -vv     # DEBUG level
gemini-ocr-tool -vvv    # TRACE level (includes library internals)
```
...
gemini-ocr-tool -v
# Output:
# [INFO] gemini-ocr-tool started
# [INFO] gemini-ocr-tool completed
...
To enable DEBUG logging for third-party libraries at TRACE level (-vvv), edit `gemini_ocr_tool/logging_config.py`:
...
eval "$(gemini-ocr-tool completion bash)"

# Zsh - active for current session only
eval "$(gemini-ocr-tool completion zsh)"

# Fish - active for current session only
gemini-ocr-tool completion fish | source
```
...
```bash
# Bash - add to ~/.bashrc
echo 'eval "$(gemini-ocr-tool completion bash)"' >> ~/.bashrc
source ~/.bashrc

# Zsh - add to ~/.zshrc
echo 'eval "$(gemini-ocr-tool completion zsh)"' >> ~/.zshrc
source ~/.zshrc

# Fish - save to completions directory
mkdir -p ~/.config/fish/completions
gemini-ocr-tool completion fish > ~/.config/fish/completions/gemini-ocr-tool.fish
```
...
```bash
# Bash
gemini-ocr-tool completion bash > ~/.gemini-ocr-tool-complete.bash
echo 'source ~/.gemini-ocr-tool-complete.bash' >> ~/.bashrc

# Zsh
gemini-ocr-tool completion zsh > ~/.gemini-ocr-tool-complete.zsh
echo 'source ~/.gemini-ocr-tool-complete.zsh' >> ~/.zshrc
...
gemini-ocr-tool completion <TAB>
# Shows: bash zsh fish
```
...
gemini-ocr-tool completion --help
```
...
make run ARGS="..."          # Run gemini-ocr-tool locally
...
# Run with coverage
uv run pytest tests/ --cov=gemini_ocr_tool
```
